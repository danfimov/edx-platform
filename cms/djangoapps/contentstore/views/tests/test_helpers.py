...
class HelpersTestCase(CourseTestCase):
    ...
    def test_xblock_studio_url(self):
        ...
        assert xblock_studio_url(self.course) == course_url

        # Verify chapter URL
        chapter = BlockFactory.create(parent_location=self.course.location, category='chapter',
                                      display_name="Week 1")
        assert xblock_studio_url(chapter) == f'{course_url}?show={quote(str(chapter.location).encode())}'
        ...
        assert xblock_studio_url(sequential) == f'{course_url}?show={quote(str(sequential.location).encode())}'

        # Verify unit URL
        vertical = BlockFactory.create(parent_location=sequential.location, category='vertical', display_name='Unit')
        assert xblock_studio_url(vertical) == f'/container/{vertical.location}'

        # Verify child vertical URL
        child_vertical = BlockFactory.create(
            parent_location=vertical.location,
            category='vertical',
            display_name='Child Vertical'
        )
        assert xblock_studio_url(child_vertical) == f'/container/{child_vertical.location}'

        # Verify video URL
        video = BlockFactory.create(parent_location=child_vertical.location, category="video", display_name="My Video")
        assert xblock_studio_url(video) is None
        # Verify video URL with find_parent=True
        assert xblock_studio_url(video, find_parent=True) == f'/container/{child_vertical.location}'

        # Verify library URL
        library = LibraryFactory.create()
        expected_url = f'/library/{str(library.location.library_key)}'
        assert xblock_studio_url(library) == expected_url
    ...
    def test_xblock_lms_url(self, mock_get_value: Mock):
        ...
        assert xblock_lms_url(sequential) == (
    ...
    def test_xblock_embed_lms_url(self, mock_get_value: Mock):
        ...
        assert xblock_embed_lms_url(chapter) == f"lms.example.com/xblock/{chapter.location}"

        # Verify sequential URL
        sequential = BlockFactory.create(
            parent_location=chapter.location, category='sequential', display_name="Lesson 1"
        )
        assert xblock_embed_lms_url(sequential) == f"lms.example.com/xblock/{sequential.location}"

    def test_xblock_type_display_name(self):

        # Verify chapter type display name
        chapter = BlockFactory.create(parent_location=self.course.location, category='chapter')
        assert xblock_type_display_name(chapter) == 'Section'
        assert xblock_type_display_name('chapter') == 'Section'

        # Verify sequential type display name
        sequential = BlockFactory.create(parent_location=chapter.location, category='sequential')
        assert xblock_type_display_name(sequential) == 'Subsection'
        assert xblock_type_display_name('sequential') == 'Subsection'

        # Verify unit type display names
        vertical = BlockFactory.create(parent_location=sequential.location, category='vertical')
        assert xblock_type_display_name(vertical) == 'Unit'
        assert xblock_type_display_name('vertical') == 'Unit'
