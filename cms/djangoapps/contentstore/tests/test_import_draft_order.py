...
class DraftReorderTestCase(ModuleStoreTestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_order(self):
        ...
        assert 7 == len(verticals)
        assert course_key.make_usage_key('vertical', 'z') == verticals[0]
        assert course_key.make_usage_key('vertical', '5a05be9d59fc4bb79282c94c9e6b88c7') == verticals[1]
        assert course_key.make_usage_key('vertical', 'a') == verticals[2]
        assert course_key.make_usage_key('vertical', 'second') == verticals[3]
        assert course_key.make_usage_key('vertical', 'b') == verticals[4]
        assert course_key.make_usage_key('vertical', 'd') == verticals[5]
        assert course_key.make_usage_key('vertical', 'c') == verticals[6]
        ...
        assert 3 == len(verticals)
        assert course_key.make_usage_key('vertical', 'asecond') == verticals[0]
        assert course_key.make_usage_key('vertical', 'secondsubsection') == verticals[1]
        assert course_key.make_usage_key('vertical', 'zsecond') == verticals[2]
